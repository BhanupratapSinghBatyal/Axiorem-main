...
// Helper to extend Standard Material efficiently
...
// Highly Optimized Geometry Generator
function createStackedPlanesBufferGeometry(n, width, height, spacing, heightSegments) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      // Direct array writes without interim allocations
      positions[posIdx++] = xOffset;
      positions[posIdx++] = y;
      positions[posIdx++] = 0;

      positions[posIdx++] = xOffset + width;
      positions[posIdx++] = y;
      positions[posIdx++] = 0;

      uvs[uvIdx++] = uvXOffset;
      uvs[uvIdx++] = uvY + uvYOffset;
      uvs[uvIdx++] = uvXOffset + 1;
      uvs[uvIdx++] = uvY + uvYOffset;
...
const MergedPlanes = forwardRef(({ material, width, count, height }, ref) => {
  ...
  const geometry = useMemo(() => {
    const geo = createStackedPlanesBufferGeometry(count, width, height, 0, 100);
    return geo;
  }, [count, width, height]);

  // Clean up geometry buffer on unmount
  useEffect(() => {
...
const Beams = ({
  ...
  // Clean up material memory when re-compiled
  useEffect(() => {

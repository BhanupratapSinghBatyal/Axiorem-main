...
function ConfirmationModal({ isOpen, type, itemName, targetWorkspaceName, error, isExecuting, onConfirm, onClose }) {
...
export default function ItemDetailsPanel({
  ...
  const { deleteProject, shareProjectToOrg, isDeleting, isSharing, deleteError, shareError } = useLibraryActions();
  const { workspace } = useOrganizationData();
  ...
  const handleConfirm = async () => {
    ...
    if (modalType === 'DELETE') {
      try {
        ...
        // Handled via deleteError in mutation hook
      }
      ...
      try {
        ...
        // Handled via shareError in mutation hook
      }
  ...
  const activeError = modalType === 'DELETE' ? deleteError : modalType === 'SHARE' ? shareError : null;
  const isExecuting = isProcessing || (modalType === 'DELETE' && isDeleting) || (modalType === 'SHARE' && isSharing);
  const targetWorkspaceName = workspace?.name || workspace?.workspaceId || 'ACTIVE WORKSPACE';
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 ${isOpen ? 'visible' : 'invisible'}`}>
        ...
        >
          ...
          <div className="flex items-start gap-4 mb-8 bg-[#212121] p-4 rounded-sm border border-slate-800">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mt-1">
                ID: <span className="text-slate-300">{item?.id || 'N/A'}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-6 overflow-y-auto pr-1">
            ...
            {item?.type && <DetailItem icon={FileText} label="Type Specification" value={item.type.toUpperCase()} />}
...
const DetailItem = ({ icon: Icon, label, value }) => (
  <div className="space-y-1">
    <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
      <Icon className="h-3.5 w-3.5 text-slate-400" /> <span>{label}</span>
    </div>
